<!DOCTYPE html>
<html lang="en">
<?php
include_once("secciones/head.php");
?>

<body id="top">
    <?php
    include_once("secciones/header.php");
    ?>
    <?php
    include_once("secciones/home.php");
    ?>
    <div class="site-section-seccion ">
        <div class="container">
            <div class="row">
                <div class="site-section-heading text-center mb-4 col-md-6 w-border mx-auto">
                    <h2 class="mb-md-4">GESTIÓN Y DESARROLLO DE RECURSOS HUMANOS</h2>
                </div>
            </div>
            <div class="row">
                <div class="text-center col-md-8 mx-auto mb-5">
                    <p> Dirección: Av. Celestino Vargas N°409, Pocollay</br>
                        Director: Lic. Adm. Javier Enrique Sotomayor Galindo</br>
                        Correo Electrónico: Electrónico: jsotomayorg@hotmail.com</br></p>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12 mt-5">
                    <table id="example" class=" table table-striped display " style="width:100%">
                        <thead class="thead-blue">
                            <tr>
                                <th class="numero">N°</th>
                                <th>PROCESO DE CAMBIO DE CARGO</th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody class="tabla">
                            <tr>
                                <td>001</td>
                                <td class="texto">CRONOGRAMA PARA ADECUACIÓN DE CARGOS</td>
                                <td class="download">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="34px" height="33px">
                                        <image x="0px" y="0px" width="34px" height="33px" xlink:href="data:img/png;base64," />
                                    </svg>
                                </td>
                            </tr> 
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
    <?php
    include_once("secciones/documentos.php");
    ?>
    <?php
    include_once("secciones/partners2.php");
    ?>
    <?php
    include_once("secciones/footer.php");
    ?>
</body>

</html>